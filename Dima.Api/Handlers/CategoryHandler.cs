...
public class CategoryHandler(AppDbContext context) : ICategoryHandler
{
    ...
    public async Task<Response<Category?>> GetByIdAsync(GetCategoryIdRequest request)
    {
        try
        {
            var category = await context
    ...
    public async Task<PageResponse<List<Category>>> GetAllAsync(GetAllCategoryRequest request)
    {
        try
        {
            ...
            return new PageResponse<List<Category>>(
                categories,
                count,
                request.PageNumber,
                request.PageSize);
        ...
        {
            return new PageResponse<List<Category>>(null, 500, "Não foi possível consultar as categorias");
        }
    }
}